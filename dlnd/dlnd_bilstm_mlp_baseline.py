...
SENT_DIM = 2048
NUM_CLASSES = 2
targets,sources,target_vecs,source_vecs,gold = pickle.load(open("dlnd_data_2048_attn.p","rb"))
...
save_file = "dlnd_ten_fold_progress_bilstm_mlp_baseline.p"
fold = 1
...
for train,test in kfold.split(np.zeros(len(gold_list)),gold_list):
	...
	train_target_vecs,valid_target_vecs,train_source_vecs,valid_source_vecs,train_golds,valid_golds = train_test_split(target_vecs[train],source_vecs[train],gold[train],train_size=0.90,random_state=956004,stratify=np.argmax(gold[train],axis=1))
